...
var ReportSolve = function(){
	...
	return {
	  	init : function() {
	  		ReportSolve.initEvent();
	  		// if(true) {
	  		// 	// 假数据
	  		// 	ReportSolve.doGenerate();
	  		// 	ReportSolve.doPlotTop();
	  		// 	return;
	  		// }
	  		ReportSolve.doRefreshData();
	  	},
		doRefreshData : function() {
			ReportSolve.hidePieRow();
			...
			var url = $.url_root+"/report/findReportIssueSolve.jspa";
	    	...
	    	$.ajax({
				...
				success: function(result){
					checkResult(result, {
	    				showBox : false,
	    				callback : function(){
	    					ReportSolve.setRawData(result.reportMap);
	    					ReportSolve.doPlotTop();
	    				}
	    			});
				},
		...
		doGenerate : function() {
			...
			ReportSolve.setRawData(data);
		},
		doPlotTop : function() {
			var $target = ReportSolve.getContainerTop();
			var series = ReportSolve.generateSeries();
			var options = {
				grid: {
				    show : true,
				    borderWidth : 0,
				...
				tooltip: true,
				tooltipOpts: {
				    content: "<b>%x</b>属于标签 <b>%s</b> 的缺陷 共有 <b>%y</b> 个", //%s -> series label, %x -> X value, %y -> Y value, %x.2 -> precision of X value, %p -> percent
				    dateFormat: "%y年%m月",
				    defaultTheme: false
				},
				xaxis: {
				...
				hooks: { 
					draw: [ReportSolve.drawCallback] 
				}
			};
			var plotObj = $.plot($target, series, options);
			ReportSolve.setPlotTop(plotObj);
			ReportSolve.doInitTable();
		},
		doInitTable : function() {
			var rawData = ReportSolve.getRawData();
			...
			for(var k = 0; k < rawData.header.length; k++) {
				var key = rawData.header[k].value;
				lastRow += common.placeholderConversion({'msg':lastRowTd,'args':[ReportSolve.calcColumnSum(key)]});
			}
		...
		generateSeries : function() {
			...
			for(var i = 0; i < rawData.header.length; i++) {
				...
				obj.data = [];
				obj.color = ReportSolve.getCustomColors(i);
				obj.bars.order = i + 1;
				...
				for(var j = 0; j < rawData.data.length; j++) {
					...
					d.push(ReportSolve.gd(year,month,2));
					d.push(value);
					obj.data.push(d);
				}
		...
		initEvent : function() {
			...
			$('#btnExport').on('click', function() {
				...
				var url = $.url_root+'/report/exportIssueSolve.jspa';
		...
		doPlotLeft : function(data) {
			...
			var $target = ReportSolve.getContainerLeft();
			var plotObj = $.plot($target, data, options);
			ReportSolve.setPlotLeft(plotObj);

			// tooltip
			ReportSolve.initTooltip($target, ReportSolve.getPlotLeft());
		},
		doPlotRight : function(data) {
			...
			var $target = ReportSolve.getContainerRight();
			var plotObj = $.plot($target, data, options);
			ReportSolve.setPlotRight(plotObj);

			// tooltip
			ReportSolve.initTooltip($target, ReportSolve.getPlotRight());
		},
		drawCallback : function(plot, canvascontext) { 
			...
			$target.find(".tickLabel").css('cursor','pointer').off('click').on('click', function(e){
    	    	...
    	    	for(var i = 0; i < rawData.data.length; i++) {
    	    		...
    	    		var $targetPieLeft = ReportSolve.getContainerLeft();
    	    		$targetPieLeft.siblings('p').find('span').text(year+"年"+(month+1)+"月");

        		    // 渲染pie chart
        		    ReportSolve.showPieRow();
    	    	}

    	    	if(dataPieLeft != null) {
    	    		ReportSolve.doPlotLeft(dataPieLeft);
    	    	}
		    	...
		    	if(dataPieRight.length != 0) {
		    		ReportSolve.doPlotRight(dataPieRight);
		    	}
			});
		...
		initTooltip : function($target, plot) {
			...
			$target.off('plothover').on('plothover', function(event, pos, item) {
			    if (item) {
			        ...
			        ReportSolve.showTooltip(pos.pageX, pos.pageY, msg);
			    } else {
			        plotObj.unhighlight();
			        ReportSolve.removeTooltip();
		...
		showTooltip : function(x, y, contents) {
			var tooltipId = ReportSolve.getTooltipId();
