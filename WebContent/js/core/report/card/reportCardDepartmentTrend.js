/**
 * FileName: reportCardDepartmentTrend.js
 *
	...
	return {
		...
		initEvent:function(){
	        ...
	        $("#btnStat").click(function(e){
	        	DepartmentTrend.stat();
	        });
	        
	        $(".exportExcel").click(function(e){
	        	var startDate=$("#EpStartDate").val();
				var endDate=$("#EpEndDate").val();
				window.location=$.url_root+'/report/exportCardDepartmentTrend.jspa?reportQueryCondition.startDate='+startDate+'&'+'reportQueryCondition.endDate='+endDate;
	        });
		},
		stat:function(){
			...
			var url=$.url_root+"/report/loadCardDepartmentTrend.jspa";
	    	$.ajax({
				url: url, 
				type : "post",
				traditional : true,
					...
					checkResult(result, {
	    				...
	    				callback : function(){
	    					var dtos=result.issueDTOList;
	    					 DepartmentTrend.generatePageGraphs(dtos);
	    					 DepartmentTrend.doInitTable(dtos);
	    				}
	    			});
				},
			...
			DepartmentTrend.generatePageGraphs(sourcedata);
			DepartmentTrend.doInitTable(sourcedata);*/
	    },
		generatePageGraphs:function(sourceData) {
	        ...
	        for(var x=0, len0 = inputDate.length;x<len0;x++){
	        	//var xdata=setData(inputDate[x].dataSource);
	       ...
	       /* function setData(iData){
				var testData=[];
				for(var i=0, len1 = iData.length;i<len1;i++){
					testData.push([Number(iData[i][0]),iData[i][1]]);
				}
				return testData;
			};*/
	        ...
	        var toggles = $("#rev-toggles_A"),
	            target = $("#flotcontainer_A");
