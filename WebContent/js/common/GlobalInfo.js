...
GlobalInfo.SpringMvcController = {
		...
		getUrl: function(config){
            if (config['proxyClass'] && config['proxyMethod']) {
                return GlobalInfo.Servlet.baseUrl+config['controller'] + '.do?method=' + config['method']
                    + '&proxyClass=' + config['proxyClass'] + '&proxyMethod=' + config['proxyMethod'] + '&jsonString=' + config['jsonString'];
            } else {
                return GlobalInfo.Servlet.baseUrl+config['controller']+'.do?method='+Math.random()+config['method'] + '&jsonString=' + config['jsonString'];
            }
		}
};
